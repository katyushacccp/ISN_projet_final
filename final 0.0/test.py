...
def melange(root,cube,can,nombre):
	...
	root.after(nombre*rFile("timeur","cub"),lambda:root.config(cursor="gumby"))
...
menuAide.add_command(label="Consulter le site web",command=lambda:webbrowser.open('http://urlz.fr/3sKM'))
...
def test():
	rotative(cube,can,1,0,"U")
	rotative(cube,can,1,0,"U")

Button(root, text="Résoudre !", command=lambda:step1(cube,can)).grid(row=2, column=1, columnspan=2)
